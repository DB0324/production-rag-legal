"""
Run recursive/structure-aware chunking over the full corpus and save results.
Run from project root: python -m src.ingestion.run_recursive_chunking
"""
import pandas as pd
from src.chunking.recursive_chunker import chunk_document_recursive
from src.chunking.chunker_base import chunks_to_dataframe

INPUT_PATH = "data/processed/corpus_final.parquet"
OUTPUT_PATH = "data/chunks/recursive_chunks.parquet"
...
def main():
    ...
    for i, row in df.iterrows():
        doc_chunks = chunk_document_recursive(
            doc_id=row["pdf_stem"],
